 ...
 * @method static \Illuminate\Database\Eloquent\Builder|Savings today()
  ...
  public function lockMoreUserFunds(Request $request, AppUser $appUser)
  {
    ...
    try {
      ...
      try {
        ...
        ErrLog::notifySuperAdmin(auth()->user(), $th, 'NewSavingsSavings failed to send');
      }

      if ($request->isApi()) return response()->json(['rsp' => 'Created'], 201);
      return back()->withFlash(['success' => 'Funds added to user´s savings']);
    } catch (\Throwable $th) {
