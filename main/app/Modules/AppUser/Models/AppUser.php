 ...
 * @property-read \Illuminate\Database\Eloquent\Collection|Savings[] $investments
...
class AppUser extends User
{
  ...
  {
    return $this->smart_savings()->active()->exists();
  }

  public function has_target_savings(): bool
  {
    return $this->target_savings()->active()->exists();
  }
  ...
  public function fund_target_savings(Savings $target_savings, float $amount): void
  {
    ...
    $target_savings->create_deposit_transaction($amount, 'Funding ' . $target_savings->portfolio->name . ' savings');
  ...
  public function defund_target_savings(Savings $target_savings, float $amount): void
  {
    ...
    $target_savings->create_withdrawal_transaction($amount, 'Automatic deduction from ' . $target_savings->portfolio->name . ' savings');
  ...
  public function validate_bvn(string $bvn, string $phone_number = null, string $full_name = null): object
  {
    ...
    try {
      ...
      ErrLog::notifySuperAdmin($this, $th, $th->getMessage());
      return (object)[
        'code' => $th->getCode(),
        'msg' => $th->getMessage()
      ];
    ...
    if ($data->mobile === get_11_digit_nigerian_number($comparison_phone_number)) {
      ...
      ErrLog::notifySuperAdmin($this, (new \Exception('BVN mismatch: ' . $bvn)), 'BVN supplied does not match supplied phone number');
      return (object)[
        'code' => 409,
    ...
    // 	];
    // } else {
    // 	ErrLog::notifySuperAdmin($this, (new \Exception('BVN mismatch: ' . $bvn)), 'BVN supplied does not match supplied full name');
    // 	return (object)[
    // 		'code' => 409,
    // 		'msg' => 'This BVN does not belong to you.'
    // 	];
